...
//---------------------------------------------------------------------------------------------------------------------
    bool request_parser::feed(http_connection_interface* connection)
    {
        if (finished())
            return true;

        http_stream_device device {connection};
        boost::iostreams::stream <http_stream_device> stream(device);
